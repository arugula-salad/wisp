#!/usr/bin/env bash
# Runs spritesd as a systemd *user* service, so the API comes back after a reboot
...
#
# Flags after `--` are spritesd's and are remembered in
# ~/.config/mini-sprites/<name>.env; without them a re-install keeps what is there.
#
#   --name NAME   unit name, default mini-sprites (a second stack needs its own)
#   --data DIR    data directory, default $MINI_SPRITES_DATA or ~/.local/share/mini-sprites
#
# Two things need root, once, and this script only tells you about them:
#
#   sudo loginctl enable-linger $USER
#       Without lingering your user manager, and spritesd in it, starts at your
#       first login and stops at your last logout.
#   sudo ./scripts/install-service.sh --system-dropin
#       Installs /etc/systemd/system/user@<uid>.service.d/mini-sprites.conf, which
...
if [ "$ACTION" = dropin ] || [ "$ACTION" = rmdropin ]; then
  ...
  if [ "$ACTION" = rmdropin ]; then
    rm -f "$DIR/mini-sprites.conf"; rmdir "$DIR" 2>/dev/null || true
    systemctl daemon-reload
    echo "ok: removed $DIR/mini-sprites.conf"
    exit 0
  fi
  mkdir -p "$DIR"
  cat > "$DIR/mini-sprites.conf" <<EOF
# Installed by mini-sprites scripts/install-service.sh --system-dropin.
# $OWNER's user manager runs spritesd. Start it after the sprite network and
...
[Unit]
After=mini-sprites-net.service mini-sprites-netd.service mini-sprites-storage.service

[Service]
TimeoutStopSec=$((STOP_TIMEOUT + 30))
EOF
  systemctl daemon-reload
  echo "ok: $DIR/mini-sprites.conf installed (ordering + a $((STOP_TIMEOUT + 30)) s stop timeout for $OWNER's user manager); takes effect the next time that manager starts, i.e. at the next boot"
  exit 0
fi
...
ENV_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/mini-sprites/$NAME.env"
LIB="$HOME/.local/lib/mini-sprites/$NAME"
...
[ -x "$REPO/bin/spritesd" ] || { echo "no $REPO/bin/spritesd: run make build (or make install-service)" >&2; exit 1; }
...
if ! systemctl --user is-active -q "$NAME.service" \
   && "$REPO/bin/spritesd" status --data "$DATA" --json 2>/dev/null | grep -q '"daemon": {'; then
  echo "a spritesd that is not $NAME.service is running on $DATA; stop it (^C suspends its sprites) and run this again" >&2
  exit 1
fi

mkdir -p "$UNIT_DIR" "$LIB" "$(dirname "$ENV_FILE")"
if [ "$HAVE_FLAGS" = 1 ] || [ ! -e "$ENV_FILE" ]; then
  { echo "# spritesd flags for $NAME.service; edit, then: systemctl --user restart $NAME"
    echo "SPRITESD_FLAGS=${FLAGS[*]}"; } > "$ENV_FILE"
fi
CUR_FLAGS="$(sed -n 's/^SPRITESD_FLAGS=//p' "$ENV_FILE")"

# The unit must not depend on a git checkout staying where it is.
install -m 0755 "$REPO/bin/spritesd" "$LIB/spritesd.new"
mv -f "$LIB/spritesd.new" "$LIB/spritesd"
...
# volume mounted spritesd would see an empty sprite directory, and without the
# bridge it would cold-boot every sprite with no NIC.
WAIT=""
if grep -qs -- " $DATA/vm\$" /etc/systemd/system/mini-sprites-storage.service; then
  WAIT+="mountpoint -q '$DATA/vm' && "
fi
if [ -e /etc/systemd/system/mini-sprites-net.service ] && ! grep -q -- '--net=false' <<<"$CUR_FLAGS"; then
...
echo "still waiting for the sprite volume or bridge (mini-sprites-storage / mini-sprites-net units)" >&2
exit 1
EOF
chmod 0755 "$LIB/wait-host.sh"

cat > "$UNIT" <<EOF
# Generated by mini-sprites scripts/install-service.sh; re-run it rather than editing.
[Unit]
Description=mini-sprites API daemon (spritesd, data in $DATA)

[Service]
Type=exec
EnvironmentFile=$ENV_FILE
ExecStartPre=$LIB/wait-host.sh
ExecStart=$LIB/spritesd --data $DATA \$SPRITESD_FLAGS
# SIGTERM goes to spritesd alone, which suspends every running sprite to disk
...
for i in $(seq 100); do
  "$LIB/spritesd" status --data "$DATA" --json 2>/dev/null | grep -q '"daemon": {' && break
...
echo "ok: $NAME.service is running and enabled (spritesd --data $DATA ${CUR_FLAGS})"
echo "    status:  $LIB/spritesd status --data $DATA"
...
if [ ! -e "/etc/systemd/system/user@$(id -u).service.d/mini-sprites.conf" ]; then
  echo
