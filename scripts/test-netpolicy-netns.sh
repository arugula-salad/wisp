...
# the exact nftables ruleset setup-host.sh installs, runs the real mini-sprites-netd
# against the real nft, starts spritesd's policy DNS + transparent proxy, and drives
...
CGO_ENABLED=0 go build -o "$OUT/mini-sprites-netd" ./cmd/mini-sprites-netd
...
podman run --rm --cap-add NET_ADMIN,SYS_ADMIN,NET_RAW --sysctl net.ipv4.ip_forward=1 \
  -v "$OUT:/ms:ro" -e MS_SETUP=/ms/setup-host.sh -e MS_NETD=/ms/mini-sprites-netd \
